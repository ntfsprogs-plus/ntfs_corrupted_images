...
cluster_duplication_a_lot/
cluster_duplication_a_lot2/
: have many clusters duplication.
...
ia_bitmap_unused_mark/
: index allocation has a cluster which is not allocated in index $Bitmap.
  cluster duplication.
...
index_bitmap_size_mismatch/
: index bitmap size mismatch.

index_block_signature_and_size_corrupted/
: index block signature and index block size of vcn 0 is not block size + orphan inodes.

index_block_signature_corrupted/
: index block signature(magic) corrupted.
...
mft_base_record_field_corrupted (deprecated)/
...
mft_mftmirr_mismatch/
: $MFT / $MFTMirr mismatch.
...
missing_mft_filename_attr2/
: filename attribute does not exist.
  orphan inodes which has corrupted mft (FILE signature is corrupted)
...
not_found_filename_tntfs_unplugged/
...
orphaned_mft_repairing/
: orphan candidate inode which has wrong mft record.
